...
const MyPageEdit = (): React.ReactNode => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const { result, error } = await PATCH(routes.api.v0.user.uri(userProfile?.id), { nickname });
    ...
    <ProtectedRoute>
      ...
      <Container.md>
        <Form onSubmit={handleSubmit}>
          ...
          <Links.Text href={routes.mypage.uri()}>취소</Links.Text>
        </Form>
      </Container.md>
    </ProtectedRoute>
  );
